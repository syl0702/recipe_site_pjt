...
def predict_ratings(**kwargs):
    ...
    ratings_matrix.to_csv(f"/home/ubuntu/airflow/dags/csv/recom_matrix_{now_time}.csv", index=True, header=True)
...
def mk_matrix(**kwargs):
    ...
    now_time = datetime.now().strftime("%Y-%m-%d")
    ratings_pred_matrix.to_csv(f"/home/ubuntu/airflow/dags/csv/recom_result_{now_time}.csv", index=['user_id', 'post_id'], header=True)
...
# # 레시피 추천하기
# def recomm_recipe_by_userid(**kwargs):
#     ti = kwargs['ti']
#     pred_df = ti.xcom_pull(task_ids='mk_matrix_task', key='ratings_pred_matrix')
#     unseen_list = ti.xcom_pull(task_ids='get_unseen_recipes_task', key='unseen_list')
#     user_id = pred_df.index.tolist()
#     recomm_recipes = pred_df.loc[user_id, unseen_list].sort_values(ascending=False)[:2]
...
# DAG 정의
dag = DAG(
    dag_id="recipe-recom-pipeline",
    schedule_interval="0 4 * * *",  # 매일 새벽 4시, 트래픽 수 가장 낮음
